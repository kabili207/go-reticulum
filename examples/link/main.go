...
func runServer(configDir *string) error {
	...
	serverDest, err := rns.NewDestination(serverID, rns.DestinationIN, rns.DestinationSINGLE, appName, "linkexample")
	if err != nil {
	...
	rns.Log("Link example "+rns.PrettyHexRep(serverDest.Hash())+" running, waiting for a connection.", rns.LogInfo)
...
func runClient(configDir *string, destinationHexHash string) error {
	...
	serverDest, err := rns.NewDestination(serverID, rns.DestinationOUT, rns.DestinationSINGLE, appName, "linkexample")
	if err != nil {
